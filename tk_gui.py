...
class User:
    def __init__(self, name, surname, posts, location):
        ...
        self.marker: object = map_widget.set_marker(self.cords[0], self.cords[0], text=f"{self.location}")
...
def show_user_details() -> None:
    ...
    map_widget.set_position(users[i].cords[0], users[i].cords[1], users[i].location)
...
def edit_user() -> None:
    ...
    entry_location.delete(0, END)
    i = listbox_lista_uzytkownikow.index(ACTIVE)
    entry_name.insert(users[i].name, END)
...
def update_user(i) -> None:
    ...
    users[i].marker = map_widget.set_marker(users[i].cords[0], users[i].cords[0], text=f"{users[i].location}")
...
map_widget = tkintermapview.TkinterMapView(ramka_szczegoly_uzytkownika, width=700, height=400)
